...
module Mastercard
  module Services
    module Match
      ...
      class TerminationInquiryService < Connector
        ...
        def get_termination_inquiry(termination_inquiry_request, options)
          body = generate_xml(termination_inquiry_request)
          url = get_url(options)
          doc = Document.new(do_request(url, 'POST', body))
          generate_return_object(doc)
        end

        def generate_xml(termination_inquiry_request)
          doc = Document.new
          doc << XMLDecl.new(1.0, 'ISO-8859-15')
          root = doc.add_element(generate_inner_xml(termination_inquiry_request))
          doc.to_s
        end

        def generate_inner_xml(termination_inquiry_request)
          el = Element.new('ns2:TerminationInquiryRequest').add_namespace('ns2', 'http://mastercard.com/termination')
          el.add_element('AcquirerId').add_text(termination_inquiry_request.acquirer_id)
          el.add_element('TransactionReferenceNumber').add_text(termination_inquiry_request.transaction_reference_number)
          merchant = el.add_element('Merchant')
          merchant.add_element('Name').add_text(termination_inquiry_request.merchant.name)
          merchant.add_element('DoingBusinessAsName').add_text(termination_inquiry_request.merchant.doing_business_as_name)
          merchant.add_element('PhoneNumber').add_text(termination_inquiry_request.merchant.phone_number)
          merchant_address = merchant.add_element('Address')
          merchant_address.add_element('Line1').add_text(termination_inquiry_request.merchant.address.line1)
          merchant_address.add_element('Line2').add_text(termination_inquiry_request.merchant.address.line2)
          merchant_address.add_element('City').add_text(termination_inquiry_request.merchant.address.city)
          merchant_address.add_element('CountrySubdivision').add_text(termination_inquiry_request.merchant.address.country_subdivision)
          merchant_address.add_element('PostalCode').add_text(termination_inquiry_request.merchant.address.postal_code)
          merchant_address.add_element('Country').add_text(termination_inquiry_request.merchant.address.country)
          merchant.add_element('CountrySubdivisionTaxId').add_text(termination_inquiry_request.merchant.country_subdivision_tax_id)
          merchant.add_element('NationalTaxId').add_text(termination_inquiry_request.merchant.national_tax_id)
          principal = merchant.add_element('Principal')
          principal.add_element('FirstName').add_text(termination_inquiry_request.merchant.principal.first_name)
          principal.add_element('LastName').add_text(termination_inquiry_request.merchant.principal.last_name)
          principal.add_element('NationalId').add_text(termination_inquiry_request.merchant.principal.national_id)
          principal.add_element('PhoneNumber').add_text(termination_inquiry_request.merchant.principal.phone_number)
          principal_address = principal.add_element('Address')
          principal_address.add_element('Line1').add_text(termination_inquiry_request.merchant.principal.address.line1)
          principal_address.add_element('Line2').add_text(termination_inquiry_request.merchant.principal.address.line2)
          principal_address.add_element('City').add_text(termination_inquiry_request.merchant.principal.address.city)
          principal_address.add_element('CountrySubdivision').add_text(termination_inquiry_request.merchant.principal.address.country_subdivision)
          principal_address.add_element('PostalCode').add_text(termination_inquiry_request.merchant.principal.address.postal_code)
          principal_address.add_element('Country').add_text(termination_inquiry_request.merchant.principal.address.country)
          principal_drivers_license = principal.add_element('DriversLicense')
          principal_drivers_license.add_element('Number')
          principal_drivers_license.add_element('CountrySubdivision')
          principal_drivers_license.add_element('Country')
          el
        end
